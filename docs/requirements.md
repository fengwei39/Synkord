...
- 约束对象：API 接口规范、团队数据模型、项目私有模型、跨项目依赖关系、接口兼容性、实体版本锁定。
...
- 类型：team、project。
- 字段：团队、名称、描述、JSON Schema、当前版本、所属项目、创建人、更新时间。
- 团队实体模型库用于统一维护当前团队的返回体、分页模型、基础字段、公共枚举。
- 项目私有模型用于服务私有 DTO、VO、请求体、响应体。
...
- 自动来源：解析 OpenAPI `$ref`。
- 手动来源：管理端手动创建依赖。
- 字段：团队、依赖方项目、被依赖项目、实体/API 名称、锁定版本、来源、创建时间。
- 每个团队拥有独立依赖关系数据，默认不混入其他团队数据。

   5.6 PostmanCollection

   表示团队导入或维护的 Postman Collection。

- 字段：团队、名称、Collection JSON、环境变量、版本、导入人、导入时间。
- 用于从 Postman Collection 生成 API 资产，并与 Swagger/OpenAPI 规范进行差异对比。

   5.7 SwaggerSpec

   表示团队维护的 Swagger/OpenAPI 文档。
...
- 字段：团队、项目、旧版本、新版本、变更人、变更时间、变更清单、影响接口、影响模型、影响项目、严重级别、站内通知状态、Webhook 发送状态。
...
- 字段：团队、通知渠道、Webhook 地址、启用状态、warning 通知开关、项目范围、最近测试结果、更新时间。
- Webhook 只负责外部群机器人通知，不参与 MCP 调用、接口管理或模型管理。
...
8. 用户有多个团队时，默认进入上次访问团队；没有访问记录时进入列表中的第一个团队。
...
2. 支持维护项目名称、描述、类型、负责人、仓库地址、OpenAPI 版本等元数据。
3. 支持按项目类型和关键字检索。
4. 支持删除、编辑、归档项目。

   6.3 接口管理

1. 支持团队内维护 HTTP API 资产。
2. 支持团队内导入 Swagger/OpenAPI JSON/YAML。
3. 支持团队内导入 Postman Collection，并解析为 API 资产。
4. 导入后自动解析 API 路径、HTTP 方法、请求参数、请求体、响应体、Schema、`$ref` 依赖。
5. 支持按当前团队内的项目、标签、路径、方法搜索 API。
6. 支持查看 API 详情，包括请求参数、响应结构、引用数据模型。
7. 支持导出项目当前 OpenAPI 规范。
8. 规范解析失败时需要返回明确错误位置和原因。
9. 支持接口级变更检测，删除接口、修改路径/方法、修改请求/响应结构均需进入变更清单。

   6.4 数据模型

1. 支持团队级数据模型库，统一维护团队返回体、分页 DTO、公共枚举、基础字段。
2. 支持团队模型、项目私有模型两层来源。
...
Webhook 是团队级通知出口，用于在接口或数据模型发生 warning / breaking 变更时，把变更摘要主动推送到钉钉、飞书等群机器人。Webhook 不参与接口管理、模型管理或 MCP 调用，只负责通知。

1. 支持团队管理员配置钉钉/飞书 Webhook。
2. warning / breaking 变更必须生成站内通知记录，顶部通知入口按当前团队展示。
3. breaking 变更必须尝试外部通知；未配置 Webhook 时记录为 not_configured，不阻断变更记录保存。
4. warning 变更支持按项目配置是否推送 Webhook。
5. info 变更默认只生成变更记录，不生成通知。
6. 通知内容包括：变更人、项目、版本、变更类型、变更路径、影响接口、影响模型、影响项目、建议处理动作。
7. 支持测试 Webhook 是否可用。
8. 通知失败需要记录失败原因，支持团队管理员手动重试。
9. Webhook 发送状态：disabled、not_configured、pending、sent、failed。
...
| 创建/编辑项目 | 按团队角色 | 是 | 是 | 否 |
| 删除项目 | 按团队角色 | 是 | 否 | 否 |
| 导入/导出 Swagger/OpenAPI/Postman | 按团队角色 | 是 | 是 | 导出 |
...
1. breaking 变更触发站内通知；配置 Webhook 后同步触发钉钉/飞书通知。
2. 通知内容包含变更人、项目、版本、变更清单、影响接口、影响模型、影响项目。
...
2. `get_team_entities` 能返回当前团队的数据模型。
