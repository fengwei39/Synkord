// Synkord ContractMembersList
// 成员列表：表格 / 角色变更 / 移除 / 邀请
// 详见 docs/ui-spec.md §七
//
// 同时支持两种使用模式：
// - embedded: true  → 嵌入到 ContractDetail 的"成员" Tab 内
// - embedded: false → 独立页（ContractMembers），含页头
...
export default function ContractMembersList({ contractId, embedded, onCountChange }: Props) {
  ...
  const columns = [
    {
      title: '成员',
      key: 'user',
      render: (_: unknown, m: ContractSetMember) => (
        <Space>
    ...
    },
    {
      ...
      width: 120,
      render: (_: unknown, m: ContractSetMember) => {
        if (m.role === 'owner') return <Text type="secondary">创建者</Text>
  ...
  return (
    <div className="contract-members-list">
      ...
      {embedded && (members?.length ?? 0) > 0 && (
        <div style={{ marginTop: 12, textAlign: 'right' }}>
          <Button type="link" icon={<UnorderedListOutlined />} onClick={() => navigate(`/contracts/${contractId}/members`)}>
            管理全部成员
          </Button>
        </div>
      )}
