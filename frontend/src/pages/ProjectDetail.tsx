...
import { ApiOutlined, ApartmentOutlined, BranchesOutlined, DatabaseOutlined, LeftOutlined } from '@ant-design/icons';
...
export default function ProjectDetail() {
  ...
  const apiParams = new URLSearchParams({ project_id: project.id }).toString();
  ...
  return (
    <div className="project-page">
      ...
      <Card title="项目资产入口">
        <Space wrap>
          <Button icon={<ApiOutlined />} onClick={() => navigate(`/apis?${apiParams}`)}>查看接口</Button>
          <Button icon={<DatabaseOutlined />} onClick={() => navigate('/models')}>查看数据模型</Button>
          <Button icon={<BranchesOutlined />} onClick={() => navigate('/diff')}>发起变更检测</Button>
          <Button icon={<ApartmentOutlined />} onClick={() => navigate('/dependencies')}>查看依赖拓扑</Button>
        </Space>
      </Card>
    </div>
  );
