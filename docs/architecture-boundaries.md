...
- 提供登录、团队、项目、接口、数据模型、成员、MCP 管理等 UI。
...
- 调用后端 REST API 获取当前激活团队和项目下的接口、模型、依赖和变更检测数据。
...
| CLI / CI / Git Hook | 后端服务 | REST 校验/导入端点 | JWT 或专用 REST Token | 推送规范、校验变更 |
...
6. 本地 MCP 服务故障不应影响后端登录、团队管理、接口管理和数据模型管理。
