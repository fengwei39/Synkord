...
5. 团队空间：团队层只保留项目管理和团队管理两个入口。
6. 项目管理：维护后端服务、Web 项目、App 项目及仓库、负责人、描述等元数据。
7. 项目详情：进入项目后管理该项目的接口、数据模型、依赖拓扑和 MCP 接入。
...
4. 每个团队拥有独立资产空间，但团队层只展示项目管理和团队管理；接口、数据模型、依赖拓扑和 MCP 管理归属到具体项目详情。
...
5. 支持我的团队列表、团队创建、团队切换、项目管理、团队管理，以及项目内接口管理、数据模型、依赖拓扑、项目 MCP 管理、本地 MCP 服务管理。
...
- 团队管理员：管理当前团队成员、团队项目，以及项目内接口、数据模型、依赖关系和当前项目 MCP Token。
